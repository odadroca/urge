            ...
            <div class="mb-4 flex items-center gap-2">
                @if($showArchived)
                    <a href="{{ route('prompts.index', array_filter(['tag' => $tag])) }}"
                       class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-md bg-amber-100 text-amber-800 border border-amber-300 hover:bg-amber-200 transition">
                        Showing archived &mdash; Hide
                    </a>
                @else
                    <a href="{{ route('prompts.index', array_filter(['tag' => $tag, 'archived' => 1])) }}"
                       class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-md bg-white text-gray-600 border border-gray-300 hover:border-gray-400 transition">
                        Show archived
                    </a>
                @endif
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="{{ $prompt->trashed() ? 'bg-gray-100 opacity-60' : '' }} hover:bg-gray-50 cursor-pointer" @click="open = !open">
                            ...
                            <td class="px-6 py-4 text-right text-sm font-medium" @click.stop>
                                @if($prompt->trashed())
                                    <span class="text-xs text-amber-600 mr-2">Archived</span>
                                    <form method="POST" action="{{ route('prompts.restore', $prompt) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium">Restore</button>
                                    </form>
                                @else
