        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('api-keys.store') }}">
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Prompt Scope <span class="text-gray-400 font-normal">(optional — leave empty for all prompts)</span></label>
                        <div class="max-h-48 overflow-auto border border-gray-300 rounded-md p-3 space-y-1">
                            @foreach($prompts as $prompt)
                            <label class="flex items-center gap-2 text-sm">
                                <input type="checkbox" name="prompt_ids[]" value="{{ $prompt->id }}"
                                    {{ in_array($prompt->id, old('prompt_ids', [])) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span>{{ $prompt->name }}</span>
                                <span class="text-gray-400 font-mono text-xs">{{ $prompt->slug }}</span>
                            </label>
                            @endforeach
                        </div>
                        @error('prompt_ids')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                    </div>
